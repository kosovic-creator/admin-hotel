...
function DodajRezrvaciju() {
	...
	const idSobaParams = params?.id as string;
	...
	useEffect(() => {
		fetch(`/api/hotel/sobe/${idSobaParams}`)
			.then((res) => res.json())
			.then(setSobe);
	}, [idSobaParams]);

	useEffect(() => {
		console.log('To su sobe', sobe);
	}, [sobe]);
  ...
  useEffect(() => {
    ...
    fetch(`/api/hotel/rezervacije/zauzeti/${idSobaParams}`)
      .then(res => res.json())
  ...
  async function novaRezervacija(e?: React.FormEvent) {
    ...
    const data = {
      sobaId: idSobaParams ? Number(idSobaParams) : undefined,
